...
public class HomOps {   
	 ...
	 * @param <T> type of image trees of the given homomorphism
	 ...
	 * @param <X> type of trees in the source range of the homomorphism
	...
	Y apply(Hom<F,G,V> hom, X startTree, TreeCreator<G,Y> tc) {
		...
		for (Tree<F> sub: subTrees1){
			subTrees2.add(apply(hom, sub, tc));
		}
	 ...
	 * is not linear, an Exception is thrown.<br>
	 * <br>
	 * Algorithm: <br>
	 ...
	 * @param <G> type of function symbols occurring in image trees of the given homomorphism
	 * @param <V> type of variables occurring in image trees of the given homomorphism
	 * @param <Q1> type of states of the finite tree automaton which the homomorphism shall be applied to
	 * @param <Q2> type of states of the resulting finite tree automaton
	 * @param <R0> type of rules of the finite tree automaton which the homomorphism shall be applied to
	 * @param <R> type of rules of the resulting finite tree automaton
	 * @param <X> type of the resulting finite tree automaton
	 * @param hom homomorphism which shall be applied to the given finite tree automaton
	 * @param ta finite tree automaton which the given homomorphism shall be applied to
	 ...
	 * @param <R> type of rules of the resulting finite tree automaton
	 * @param <X> type of the resulting finite tree automaton
	 * @param rule regarded rule
	 * @param tree regarded subtree
	 * @param actPos regarded actual position in the tree
	 * @param newStates set that contains already created new states, in which newly created states must be inserted
	 * @param newRules set that contains already created new rules, in which newly created rules must be inserted
	 * @param newEpsRules epsilon rules for the finite tree automaton with epsilon rules that contains already created rules
	 * @param actDestState already converted state of the regarded rule
	 * @param map map to remember the mapping between old and new states
	...
	void createRulesForAutomaton(
		...
		for (Tree<? extends BiSymbol<G,V>> sub: tree.getSubTrees()){
			...
			// recursion
			HomOps.<F,G,V,Q1,R0,Q2,R,X>createRulesForAutomaton(rule,sub,newPos,newStates,newRules,newEpsRules,qnew,map,c2,fc);
	 ...
	 * <li> For each symbol f with arity n and hom(f){x_1<-p_1, ... x_n<-p_n} ->* q
	 ...
	 * @param <U> type of trees with symbols of type G
	 * @param <V0> type of configuration trees associated with the given finite tree automaton
	 * @param <Q1> type of states of the given finite tree automaton
	 * @param <Q2> type of states of the resulting finite tree automaton
	 * @param <R> type of rules of the resulting finite tree automaton
	 * @param <Y> type of the resulting finite tree automaton
	 * @param hom homomorphism to be inverse-applied to the given finite tree automaton
	 * @param ta finite tree automaton which describes the regular tree language
	 ...
	 * @param <Q> type of states which the variables in the given variable tree shall be replaced by
	 * @param tree variable tree whose variable shall be replaced by given states
	...
	boolean isSymbolToSymbol(Hom<F,G,V> hom){
		...
		for (F f: hom.getSrcAlphabet()){
			if (TreeOps.getHeight(hom.imageOf(f)) != 1) return false;
		}
		return true;
	}
	 ...
	 * @param <V> type of variables occurring in the image trees of the given homomorphism
	 * @param hom homomorphism to be analyzed
	 * @return true if and only if the given homomorphism is alphabetic,
